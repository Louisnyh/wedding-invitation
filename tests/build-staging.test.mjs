...
test('retired legacy builder refuses to package the mixed public-cutover root',async()=>{
 await assert.rejects(build(),/Forbidden legacy surface/);
});
...
test('retired legacy preview refuses to serve the mixed public-cutover root',async()=>{
 await assert.rejects(startStaging({port:0}),/Forbidden legacy surface/);
});
...
test('frontend v2 builds an exact independent allowlist without touching the legacy output',async()=>{
 ...
 assert.match(html,/class="ceremony"[\s\S]*data-wedding-field="event\.dateDisplay"[\s\S]*data-wedding-field="schedule\.receptionTime"[\s\S]*data-wedding-field="schedule\.dinnerTime"[\s\S]*data-wedding-field="venue\.venueName"[\s\S]*data-wedding-field="venue\.dressCode"/);
 assert.match(html,/data-wedding-map[\s\S]*target="_blank"[\s\S]*rel="noopener noreferrer"/);
 ...
 assert.match(eveningHtml,/\b06\b[\s\S]*The Evening/);
 assert.equal([...eveningHtml.matchAll(/data-wedding-timeline/g)].length,4);
 ...
 assert.equal(new URL('assets/photos/hero-main.webp',`https://example.invalid/wedding-invitation/${namespace}/frontend-v2/`).pathname,`/wedding-invitation/${namespace}/frontend-v2/assets/photos/hero-main.webp`);
